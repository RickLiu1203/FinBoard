...
app.post('/balances', async function(req, res, next){
  try{
      const access_token = req.body.access_token;
      const plaidRequest = {
          access_token: access_token,
...
app.post('/payment', (req, res) => {
  ...
  console.log(`Received payment request for contact: ${contact} and amount: ${amount}`);

  res.json({ message: 'Payment request received successfully.' });
});

app.post('/income', async function(req, res, next){
  try{
      const access_token = req.body.access_token;
      const plaidRequest = {
          access_token: access_token,
      ...
      const plaidResponse = await plaidClient.creditBankIncomeGet(plaidRequest);
      res.json(plaidResponse.data);
  } catch (error) {
      res.status(500).send("failure")
  }
